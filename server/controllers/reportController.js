...
export const getMonthlyReport = async (req, res) => {
  try {
    ...
    if (req.query.format === 'pdf') {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename=finance-report-${report.period}.pdf`
      );
      ...
      report.transactions.slice(0, 40).forEach((item) => {
        doc
          .fontSize(10)
          .text(
            `${new Date(item.transactionDate).toLocaleDateString('en-IN')} | ${item.type} | ${item.title} | ${formatCurrency(item.amount)}`
          );
      });
