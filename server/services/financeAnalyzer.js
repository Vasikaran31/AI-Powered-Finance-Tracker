...
export const analyzeFinance = ({ transactions = [], budget = null }) => {
  ...
  if (!transactions.length) {
    return {
      ...
      recommendations: [
        'Start with recurring expenses like rent, subscriptions, groceries, and transport.'
      ],
      predictedExpense: 0,
      confidence: 0
    };
  ...
  if (topCategory) {
    const share = summary.totalExpense
      ? Math.round((topCategory.amount / summary.totalExpense) * 100)
      : 0;
    insights.push(
      `${topCategory.category} is your biggest spending category at ${currency(topCategory.amount)} (${share}% of expenses).`
    );
  }
  ...
  if (summary.monthlyBudget > 0) {
    ...
    if (used >= 100) {
      insights.push(
        `You have exceeded your monthly budget by ${currency(Math.abs(summary.budgetRemaining))}.`
      );
    } else if (used >= 80) {
      insights.push(
        `You have used ${used}% of your monthly budget. Slow down discretionary spending this month.`
      );
    } else {
      insights.push(
        `You still have ${currency(summary.budgetRemaining)} available from this month's budget.`
      );
    }
  }
  ...
  budget?.categoryBudgets?.forEach((item) => {
    const spent =
      summary.categoryBreakdown.find((entry) => entry.category === item.category)?.amount || 0;
    if (item.limit > 0 && spent > item.limit) {
      insights.push(
        `${item.category} spending exceeded its category budget by ${currency(spent - item.limit)}.`
      );
    }
  });
  ...
  if (summary.savings < 0) {
    insights.push(
      `Your expenses are higher than income by ${currency(Math.abs(summary.savings))}.`
    );
    recommendations.push(
      'Prioritize reducing flexible categories until monthly cash flow is positive again.'
    );
  } else {
    recommendations.push(
      `You are currently saving ${currency(summary.savings)}. Consider moving part of it to a dedicated savings goal.`
    );
  }

  if (topCategory) {
    recommendations.push(
      `A 10% reduction in ${topCategory.category} could save about ${currency(topCategory.amount * 0.1)} this period.`
    );
  }
